...
function Test-SafeToDeleteSourceFile {
    ...
    if (-not (Test-PathWithinAllowedRoots -Path $SourcePath -AllowedRoots $AllowedRoots)) {
        return [PSCustomObject]@{ Safe = $false; Reason = 'source path is not within SourceCleanup.AllowedRoots' }
    }
...
<#
.SYNOPSIS
Resolves SourceCleanup configuration.
.DESCRIPTION
    SourceCleanup = @{ Enabled = $true; AllowedRoots = @('\\host\share\folder') }
Returns @{ Enabled; AllowedRoots }. A missing/empty SourceCleanup section
yields Enabled=$true with empty AllowedRoots, so the asserter fails closed
(SourceCleanup being enabled with nothing whitelisted is a config error, not
a silent no-op) rather than defaulting to disabled.
#>
