...
public static class XmlPhotoReader
{
    ...
    /// 不同 Personnel 的非空 Text2 必须唯一(去除首尾空白、忽略大小写),重复时抛出 InvalidOperationException。
    /// </summary>
    ...
    /// 成功返回前已读到文档末尾;格式错误或重复 MSID 都会在调用方写盘前抛出。
    /// </summary>
        ...
        Action<PersonnelInfo>? onPersonnel = null)
    {
        ...
        var seenMsids = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            ...
            if (reader.NodeType == XmlNodeType.Element)
            {
                if (reader.LocalName == PersonnelElement)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(msidForWarn) && !seenMsids.Add(msidForWarn))
                        throw new InvalidOperationException($"XML 出现重复 Personnel MSID '{msidForWarn}',拒绝导入。");
                    if (extraImages)
