...
public sealed class PhotoImportJob
{
    ...
    private void SaveXmlAudit(XmlPhotoAudit audit, string status, RunSummary summary)
    {
        try
        {
            var directory = string.IsNullOrWhiteSpace(_opt.XmlAuditDirectory)
                ? Path.Combine(Path.GetDirectoryName(Path.GetFullPath(_manifestPath))!, "xml-audit")
                : _opt.XmlAuditDirectory;
        ...
        XmlPhotoAudit audit)
    {
        ...
        using (stream)
        {
            ...
            foreach (var rec in metadata)
            {
                ...
                {
                    audit.Set(msid, "Error", "MissingLastModifiedTime");
                ...
                {
                    audit.Set(msid, "Error", "InvalidLastModifiedTime");
                ...
                catch (Exception ex) { audit.Set(msid, "Error", "InvalidDestination"); _log.LogWarning(ex, "算路径失败(xml) msid={Msid}", msid); s.Errors++; continue; }
