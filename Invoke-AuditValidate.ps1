...
$summary = [PSCustomObject]@{
    ...
    # Explicit, not inferred: consumers (e.g. the ops completion notification)
    # must not guess "was cleanup enabled" from ArchiveResult's nullness - that
    # ties them to this script's internal branching and silently breaks if it
    # is ever restructured.
    SourceCleanupEnabled = [bool]$sourceCleanupConfig.Enabled
}
