...
.PARAMETER ValidatePendingWatchdogSeconds
Slow-path quiet period used instead of DebounceSeconds specifically while the
Validate manifest is resolved-but-incomplete and everything present belongs
to it - long enough that routine staggered .msg submissions between BUs don't
each trigger a premature (if accurate) preflight-missing-file notification,
short enough that a genuinely stuck cycle still surfaces same-day, well
before the 18:00 FinalCheck escalation. Default 1800 (30 minutes).
...
param(
    ...
    [int]$ValidatePendingWatchdogSeconds = 1800,
...
function Update-WatcherState {
    ...
    One poll step. Mutates $State in place, returns the action list:
      ...
      'SlowKick' fires after DebounceSeconds normally, but after the longer
      ValidatePendingWatchdogSeconds instead while the Validate manifest is
      resolved-but-incomplete and every file present belongs to it (see the
      slow path block below) - a genuinely stuck cycle still surfaces, just
      not on every routine staggered-arrival gap between BUs.
    $State keys: LastSnapshot, Stability, LastChangeAt, Armed, FastKicked,
                 ...
                 ValidatePendingWatchdogSeconds
    #>
    param(
    ...
    if (-not $State.ValidateFastKicked -and $State.ValidateManifestResolved -and
        ...
        if (Test-AnalysisSetReady -Snapshot $Snapshot -Stability $State.Stability -ExpectedNames $State.ValidateExpectedNames) {
            ...
            # Disarm the slow path, same as the Analysis fast path above:
            # otherwise the .msg batch that just satisfied this channel is
            # still "Armed" from its own arrival and produces a redundant
            # SlowKick once DebounceSeconds elapses - defeating the point of
    ...
    # Slow path: global quiet period after any activity. When the Validate
    # manifest is already resolved and everything currently in the folder
    # belongs to it (nothing unexpected/misnamed), a routine few-minute gap
    # between staggered .msg submissions from different BUs would otherwise
    # fire a "file(s) missing" preflight notification that resolves itself
    # minutes later on its own - accurate at the moment, but premature. Use a
    # longer watchdog interval for that specific case instead of suppressing
    # the safety net outright: a genuinely stuck cycle (the file really never
    # arrives) still gets an ops heads-up well before the 18:00 FinalCheck
    # escalation, just not on every routine staggered-arrival gap. Any
    # unexpected/misnamed file, or an unresolved/unresolvable manifest, keeps
    # the normal (short) DebounceSeconds as the safety net.
    $requiredQuietSeconds = $State.DebounceSeconds
    if ($State.Armed -and $State.ValidateManifestResolved -and -not $State.ValidateFastKicked -and
        @($State.ValidateExpectedNames).Count -gt 0) {
        ...
        if (-not $hasUnexpectedFile) {
            $requiredQuietSeconds = [Math]::Max($State.DebounceSeconds, $State.ValidatePendingWatchdogSeconds)
        }
    }
...
$state = @{
    ...
    ValidatePendingWatchdogSeconds = $ValidatePendingWatchdogSeconds
}
