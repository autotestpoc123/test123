...
function Send-AuditEscalationNotification {
    ...
    $body = Build-AuditNotificationHtml `
        -Heading "WeCom Audit ESCALATION - Cycle $CycleEndDate not completed" `
        -Intro "Cycle $(& $enc $CycleStartDate) - $(& $enc $CycleEndDate) was not completed by the 18:00 deadline. Pending stage: $(& $enc $PendingStage)." `
        -Sections $sections `
        -Footer 'This is the formal deadline-miss record for this cycle. Complete the pending stage (drop the outstanding files into the source folder), or engage engineering if the cycle cannot be recovered.'

    $envTag = ([string]$Environment).ToUpperInvariant()
    $subject = "[WeCom Audit][$envTag] ESCALATION - Cycle $CycleEndDate not completed (pending: $PendingStage)"
