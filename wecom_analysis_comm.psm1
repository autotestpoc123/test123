...
function Test-BackupFolderContent {
    ...
    $expectedSet = New-Object 'System.Collections.Generic.HashSet[string]' $comparer
    foreach ($f in $ExpectedFiles) { $null = $expectedSet.Add($f) }
    $actualSet = New-Object 'System.Collections.Generic.HashSet[string]' $comparer
    foreach ($f in $actualFiles) { $null = $actualSet.Add($f) }

    $missingFiles = @($ExpectedFiles | Where-Object { -not $actualSet.Contains($_) })
    $unexpectedFiles = @($actualFiles | Where-Object { -not $expectedSet.Contains($_) })
...
function Build-AuditNotificationHtml {
    ...
    foreach ($section in $Sections) {
        ...
        $lines.Add("<h4>$title ($($items.Count))</h4><ul>")
    ...
    if ($Footer) { $lines.Add("<p>$Footer</p>") }
    return ($lines -join "`n")
}
...
function Send-ValidationFailureNotification {
    ...
    $sections = @(
        [PSCustomObject]@{ Heading = 'Missing Files';    Items = @($MissingFiles    | ForEach-Object { "<b>$(& $enc $_)</b>" }) }
