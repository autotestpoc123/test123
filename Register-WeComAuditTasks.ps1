...
Register-AuditTask -TaskName 'WeComAudit-AutoCycle' `
    -Arguments "-NoProfile -ExecutionPolicy Bypass -File `"$schedulerPath`" -ConfigPath `"$ConfigPath`""

# 2) SourceWatcher: cycle Thursdays 10:00, self-terminates at 18:00.
Register-AuditTask -TaskName 'WeComAudit-SourceWatcher' `
    -Arguments "-NoProfile -ExecutionPolicy Bypass -File `"$watcherPath`" -ConfigPath `"$ConfigPath`"" `
...
Register-AuditTask -TaskName 'WeComAudit-FinalCheck' `
    -Arguments "-NoProfile -ExecutionPolicy Bypass -File `"$schedulerPath`" -ConfigPath `"$ConfigPath`" -Escalate" `
...
Write-Host "All task actions pinned to config: $ConfigPath" -ForegroundColor Green
