...
function Send-ValidationFailureNotificationFromSummary {
    ...
    $summaryPath = Join-Path $RunsRoot (Join-Path $RunId 'validation\backup-validation-summary.json')
    ...
    $missingFiles = @()
    $unexpectedFiles = @()
    if ($summary.PSObject.Properties['MissingFiles'])    { $missingFiles    = @($summary.MissingFiles    | ForEach-Object { [string]$_ }) }
    ...
    try {
        $notificationBlock = [PSCustomObject]@{
            Attempted  = $attempted
            Sent       = $sent
        ...
        $summary | Add-Member -MemberType NoteProperty -Name 'Notification' -Value $notificationBlock -Force
...
if ($Phase -in @('Validate', 'All')) {
    ...
    Invoke-PreflightCheck `
        ...
        -EndDate $cycle.EndDate `
        -Environment $env `
        -BackupFolder $resolvedSourceFolder
