...
function Test-CompletionNotificationDueForRetry {
    <#
    Pure decision, no I/O: should the validateGuard.IsComplete branch retry
    the ops completion notification for an already-complete cycle? Never
    sends anything - the caller does that. Takes MarkerExists/SummaryExists/
    SummaryLastWriteTime/Now as plain inputs (not Test-Path/Get-Item/Get-Date
    calls) specifically so this is directly unit-testable without a real
    filesystem.

    Rules:
      - marker already exists            -> not due (already sent, done)
      - no summary at all                -> not due (nothing to retry from)
      - summary exists, never attempted before (no reliable mtime) -> due
      - summary exists, last attempt (its own mtime - every send attempt,
        success or failure, rewrites the Notification block and bumps it)
        was at least ThrottleMinutes ago -> due
      - otherwise (attempted too recently) -> not due, avoids hammering a
        persistently broken SMTP endpoint on every redundant kick
    #>
    param(
        [Parameter(Mandatory)][bool]$MarkerExists,
        [bool]$SummaryExists,
        $SummaryLastWriteTime,
        [Parameter(Mandatory)][datetime]$Now,
        [int]$ThrottleMinutes = 15
    )
    if ($MarkerExists) { return $false }
    if (-not $SummaryExists) { return $false }
    if ($null -eq $SummaryLastWriteTime) { return $true }
    return (($Now - [datetime]$SummaryLastWriteTime).TotalMinutes -ge $ThrottleMinutes)
}
...
try {
    ...
    if ($validateGuard.IsComplete) {
        ...
        # only (maybe) resends the notification.
        $completionMarkerPath = Get-ValidateCompletionMarkerPath -RunsRoot $runsRoot -RunId $validateGuard.RunId
        $completionMarkerExists = Test-Path -LiteralPath $completionMarkerPath -PathType Leaf
        if (-not $completionMarkerExists) {
            $completionSummaryPath = Get-ValidateSummaryPath -RunsRoot $runsRoot -RunId $validateGuard.RunId
            $completionSummaryExists = Test-Path -LiteralPath $completionSummaryPath -PathType Leaf
            $completionSummaryMtime = if ($completionSummaryExists) { (Get-Item -LiteralPath $completionSummaryPath).LastWriteTime } else { $null }

            if (Test-CompletionNotificationDueForRetry -MarkerExists $completionMarkerExists `
                    -SummaryExists $completionSummaryExists -SummaryLastWriteTime $completionSummaryMtime -Now (Get-Date)) {
            ...
            elseif ($completionSummaryExists) {
                Write-Host "Completion notification for RunId '$($validateGuard.RunId)' still unsent, but the last attempt was recent; not retrying yet." -ForegroundColor DarkGray
            }
        }
