...
默认目录为 applied-manifest 所在目录下的 `xml-audit`，可通过 `PhotoImport:XmlAuditDirectory` 配置独立目录。
文件名带 UTC 时间和唯一 ID，不覆盖历史报表。目录需要写权限；报表含 MSID，应限制访问并由运维制定保留/清理策略。

CSV 第一行为列名，第二行为 `RowType=Run` 的状态记录，其余 `RowType=Personnel` 为第一遍扫描遇到的人员。
...
- `ScanComplete`：是否完整通过第一遍结构及重复 MSID 校验。False 的报表可能只有部分人员，不能对账全量。
- `DryRun`：是否演练。
...
- `HasImage`：第一 Images 块是否含非空 Image，不代表 Base64 或图片格式已验证。不读取 Thumbnail。
- `Result / Reason`：Written、WouldWrite、Skipped、Error、Planned 及具体原因。NotEvaluated/Planned 在失败或取消时可能保留，不能视为成功。

在 Excel 中只筛选 `RowType=Personnel`、`IsValidMsid=True` 且 `ScanComplete=True` 的行，按 IsActive / HasImage 四种组合计数：
...
四类相加等于本次扫描的有效 MSID 人数。无效或缺失 MSID 单独统计。
...
XML 第一遍失败或重复 MSID 时输出 Failed/False，已收集的人员只是部分输入；不会因此放宽拒绝导入策略。
报表通过临时文件写完后重命名，避免将半写文件当成完成报表。CSV 写入失败计入 Errors，阻止本轮水位推进；已完成照片写入不回滚。
未访问图片内容或再次扫描 NAS；只在第一遍扫描中收集轻量人员字段，内存开销随 XML 人员数量增长。
